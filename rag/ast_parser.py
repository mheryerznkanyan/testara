...
def _extract_via_tree_sitter_swiftui(body_code: str) -> Optional[List[Dict]]:
    ...
    try:
        import tree_sitter_swift as _ts_swift_mod
    except ImportError:
        return None

    try:
        lang = TSLanguage(_ts_swift_mod.language())
...
def extract_swiftui_elements(view_name: str, body_code: str) -> List[Dict]:
    ...
    ----------
    view_name:
        The SwiftUI struct/view name (e.g. ``"LoginView"``).
        Used for logging/debugging; not included in output.
    body_code:
        The full text of the SwiftUI view block (struct body).
    ...
    """
    ts_result = _extract_via_tree_sitter_swiftui(body_code)
