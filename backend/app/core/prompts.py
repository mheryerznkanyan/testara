...
SPRINGBOARD ALERTS — handle system popups that block tests:
```swift
...
FORBIDDEN PATTERNS:
- NEVER use app.otherElements[] — fails for SwiftUI views
- NEVER use NSPredicate, .matching(NSPredicate(...)), or .containing(NSPredicate(...))
- NEVER hardcode data values in XCTAssertEqual unless 100% certain
- NEVER guess accessibility IDs — use ONLY IDs from RAG context
- NEVER use compound boolean expressions in XCTAssertTrue

DATA ASSERTIONS:
- Verify element EXISTS rather than checking content — use XCTAssertTrue(element.exists)
- NEVER check .label.isEmpty on cells or list items — SwiftUI cells often have empty labels
- NEVER use XCTAssertEqual with hardcoded strings unless 100% certain of the value
